...
export type itemComponentsData={beginTijd:string,eindTijd:string,datum:string, itemId:string, UserData:{naam:string,userId:number}[]}
    ...
    returnFormatedSortedJSON=(json:formatedRoosterItems):fullRenderItem=>{
        ...
        Object.keys(formatJson).forEach(value => {
            ...
            ObjectList.forEach((value1, index) => {
                ...
                formatJson[value][value1]=this.retrurnRenderdItems({beginTijd:beginTijd,eindTijd:eindTijd,datum:value, itemId:value, UserData:values},itemStyleData[index]&&itemStyleData[index].width+"%",itemStyleData[index]&&itemStyleData[index].start+"%")
            })
